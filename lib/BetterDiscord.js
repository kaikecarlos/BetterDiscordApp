    ...
    return new Promise(resolve => {
        utils.getHash(_cfg.repo, _cfg.branch, function(hash) {
            if (!hash)  {
                ...
                hash = "48844445d65c6fb5a019eff14d7dcffcc1744071";
            }
            _cfg.hash = hash;
...
function updateExtData() {
    _extData = {
        ...
        'load-mainJS': {
            ...
            'resource': 'Main JS',
            'domain': _cfg.updater.CDN,
            'url': '//' + _cfg.updater.CDN + '/' + _cfg.repo + '/BetterDiscordApp/' + _cfg.hash + '/js/main.js?v=1.1',
...
function load(reload) {
    ...
    if(!reload) {
        if(_cfg.updater.LatestVersion > _cfg.version) {
            utils.alert("Update Available", "An update for BandagedBD is available ("+_cfg.updater.LatestVersion+")! <a href='https://github.com/kaikecarlos/BetterDiscordApp/releases' target='_blank'>Download Installer</a>");
        }
    ...
    try {
        ...
        utils.execJs('var themesupport2 = true');
